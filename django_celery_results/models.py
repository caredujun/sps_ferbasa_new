...
class TaskResult(models.Model):
    ...
    def status_real(self):
        return self.status
    status_real.short_description = 'Status'

    def task_name_real(self):
        return self.task_name
    task_name_real.short_description = 'Task Name'

    def duration_task(self):
        if self.date_done is not None and self.date_created is not None:
            delta = (self.date_done  - self.date_created).total_seconds()/60
            return locale.format_string('%.2f', delta, True)
        else:
            return ''

    duration_task.short_description = 'Duration (min)'
